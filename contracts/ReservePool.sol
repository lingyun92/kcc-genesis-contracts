...
contract ReservePool is Params, Admin, IReservePool {
    ...
    returns(uint256) {
        ...
        require(_rewardWithdrawnRecords[block.number] == 0,"multiple withdrawals in a single block");
    ...
    external 
    onlyAdmin{
        require(newState == State.DISABLED || newState == State.ENABLED, "invalid state");
        state = newState;
    }


    // Set the new block reward amount 
    function setBlockRewardAmount(uint256 amount)
    external
    onlyAdmin{
        require(amount < MAX_BLOCK_REWARD_AMOUNT,"amount is greater than maximum");
